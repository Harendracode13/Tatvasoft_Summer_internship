...
export class RegisterComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    if (this.registerForm.valid) {
      ...
      const registerSubscribe = this._service.registerUser(register).subscribe({
        next: (data: any) => {
          if (data.result == 1) {
        ...
        error: (error) => {
          if (error.error && error.error.message) {
            this._toast.error({ detail: "ERROR", summary: error.error.message, duration: APP_CONFIG.toastDuration });
          } else {
            this._toast.error({ detail: "ERROR", summary: "Registration failed. Please try again.", duration: APP_CONFIG.toastDuration });
          }
        }
      });
